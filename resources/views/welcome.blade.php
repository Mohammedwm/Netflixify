                            ...
                            <div class="movie__cta my-4">
                                ...
                                @auth
                                    <a href="#" class="btn btn-outline-light text-capitalize movie__fav-btn">
                                        <span class="far fa-heart movie__fav-icon {{ $latest_movie->is_favored ? 'fw-900' : ''}}  movie-{{ $latest_movie->id }}"
        ...
        <div class="container">
            ...
            <div class="row my-4">
                <div class="col-12 d-flex justify-content-between">
                    <h3 class="listing__title text-white fw-300">Most Watched</h3>
                </div>
            </div><!-- end of row -->

            <div class="movies owl-carousel owl-theme">

                @foreach($view_movies as $view_movie)
                    <div class="movie p-0">
                        <img src="{{ $view_movie->poster_path}}" class="img-fluid" alt="">

                        <div class="movie__details text-white">

                            <div class="d-flex justify-content-between">
                                <p class="mb-0 movie__name">{{ $view_movie->name }}</p>
                                <p class="mb-0 movie__year align-self-center">{{ $view_movie->year }}</p>
                            </div>

                            <div class="d-flex movie__rating">
                                <div class="mr-2">
                                    @for ($i = 0; $i < $view_movie->rating; $i++)
                                        <span class="fas fa-star text-primary mr-1"></span>
                                    @endfor
                                </div>
                                <span>{{ $view_movie->rating }}</span>
                            </div>

                            <div class="movie___views">
                                <p>Views : {{ $view_movie->views }}</p>
                            </div>

                            <div class="d-flex movie__cta">
                                <a href="{{ route('movies.show', $view_movie->id) }}" class="btn btn-primary text-capitalize flex-fill mr-2"><i class="fas fa-play"></i> watch now</a>
                                
                                @auth
                                    <i class="far fa-heart {{ $view_movie->is_favored ? 'fw-900' : ''}} fa-1x align-self-center movie__fav-icon movie-{{ $view_movie->id }}"
                                        data-id = "{{ $view_movie->id }}"
                                        data-url= "{{ route('movies.toggle_favorite',$view_movie->id) }}"
                                    ></i>
                                @else 
